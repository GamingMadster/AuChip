...
void Execute(int op) {
  ...
  int enabled2 = (chip.planeSelect & 2) >> 1;
      ...
      if(SuperChip){
        if(XoChip){
          ...
          for(int i = 0; i<v0001; i++){
            ...
            if(W==16)Pixels = (chip.memory[(i*2+chip.I)&0xFFFF]<<8)+chip.memory[(i*2+1+chip.I)&0xFFFF];
